...
data.forEach((item) => {
  let sectionItem = listArr.find((q) => q.firstletter == item.firstName[0]);
  if (sectionItem) {
      ...
      <View style = {styles.container}>
        <SectionList
          ...
          renderSectionHeader={({ section: { firstletter } }) => (
            <View style={{ paddingHorizontal: 20, backgroundColor: "#EFEFEF"}}>
              <Text style={{ fontSize: 20, fontWeight:'500', paddingVertical:5, }}>{firstletter}</Text>
            </View>
          )}
        />
      </View>
...
const renderItem = ({ item }) => {
  return (
    <View style={{ paddingHorizontal: 20, marginVertical: 5 }}>
      <Text style={{ fontSize: 16, paddingVertical: 5 }}>{item}</Text>
    </View>
  );
}
